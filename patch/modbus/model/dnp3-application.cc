...
Dnp3Application::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::Dnp3Application")
    ...
    .AddAttribute ("RemoteAddress2",
		    ...
		    MakeAddressAccessor (&Dnp3Application::m_remoteAddress2),
		    MakeAddressChecker())
	.AddAttribute ("RemotePort",
	               ...
	               MakeUintegerChecker<uint16_t> ())
	.AddAttribute ("MasterPort", "The Master's destination port", UintegerValue (0), MakeUintegerAccessor (&Dnp3Application::m_masterport), MakeUintegerChecker<uint16_t>())
    ...
    .AddAttribute ("PointsFilename",
                   ...
                   StringValue (),
                   MakeStringAccessor (&Dnp3Application::points_filename),
                   MakeStringChecker ())
    ...
    .AddTraceSource ("Rx2", "A packet has been received",
                       MakeTraceSourceAccessor (&Dnp3Application::m_rxTraces),
                      "ns3::Packet::TracedCallback")
    .AddTraceSource ("RxWithAddresses", "A packet has been received",
                       MakeTraceSourceAccessor (&Dnp3Application::m_rxTraceWithAddresses),
    ...
    .AddAttribute ("mitmFlag", "Man in the middle flag",
		    BooleanValue (false),
		    MakeBooleanAccessor (&Dnp3Application::mitm_flag),
		    MakeBooleanChecker())
  ;
  return tid;
}
...
//Added TWE
    ...
    m_socket->SetRecvCallback (MakeCallback (&Dnp3Application::HandleRead, this));
    mim_socket->SetRecvCallback (MakeCallback (&Dnp3Application::HandleRead, this));
    if(m_isMaster == true) {
...
//    else{
        ...
        if(running) {
            ...
            Simulator::Schedule(MilliSeconds(4000), &Dnp3Application::periodic_poll, this, ++count); //virtual method
        }
//    }
}

void Dnp3Application::periodic_poll_Integrity(int pollRate){
  if(running){
    NS_LOG_INFO("INSIDE periodic_poll_Integrity");
    m_p->poll(Master::INTEGRITY);
    Simulator::Schedule(MilliSeconds(pollRate), &Dnp3Application::periodic_poll_Integrity, this, pollRate); //virtual method
  }

}

void Dnp3Application::periodic_poll_Binary(int pollRate){
  if(running){
    NS_LOG_INFO("INSIDE periodic_poll_Binary");
    m_p->poll(Master::BINARY);
    Simulator::Schedule(MilliSeconds(pollRate), &Dnp3Application::periodic_poll_Binary, this, pollRate);
  }

}

void Dnp3Application::periodic_poll_Analog(int pollRate){
  if(running){
    NS_LOG_INFO("INSIDE periodic_poll_Analog");
    m_p->poll(Master::ANALOG);
    Simulator::Schedule(MilliSeconds(pollRate), &Dnp3Application::periodic_poll_Analog, this, pollRate);
  }
}

            // NS_LOG_INFO("Binary name: " << name);
void Dnp3Application::periodic_poll_Class(int pollRate){
  if(running){

    NS_LOG_INFO("INSIDE periodic_poll_Class");
    m_p->poll(Master::CLASS);
    Simulator::Schedule(MilliSeconds(pollRate), &Dnp3Application::periodic_poll_Class, this, pollRate);
  }
...
void Dnp3Application::send_device_attribute_request(DnpIndex_t index) {
...
void Dnp3Application::set_respond(bool respond) {
...
void Dnp3Application::set_offline(bool offline) {
...
void Dnp3Application::set_multiplier(float multiplier) {
...
void Dnp3Application::handle_inside(Ptr<Socket> socket) {
...
void Dnp3Application::send_directly_server(Ptr<Packet> p)
{
   m_txTrace (p);
...
void 
Dnp3Application::save_data(Ptr<Socket> socket, Ptr<Packet> packet, Address from)
{
...
void Dnp3Application::calc_crc(Bytes buf_temp, uint8_t * temp2, Ptr<Packet> testPack){
...
std::vector<std::string> Dnp3Application::get_val_vector (std::string delimiter, std::string m_attack_val){
    size_t pos = 0;
...
int Dnp3Application::start_byte(uint8_t * temp2, Ptr<Packet> testPack){
    int start;
...
float Dnp3Application::get_val(std::vector<std::string> val, std::vector<std::string> val_min, std::vector<std::string> val_max, int index){
    float f = 0.0;
    if (index < val_min.size() && index < val_max.size()){
        if (val_min[index] != "NA" && val_max[index] != "NA"){
    ...
    while ((packet = socket->RecvFrom (from)))
    {
     m_txTrace(packet);
      if(mitm_flag == true) {
		  ...
		  //UdpHeader udpHeader;
		  //testPack->RemoveHeader(udpHeader);
		  ...
		  //end
		     ...
		     NS_LOG_INFO("Getting the first item of buf");
                     ...
                     if((start1 == 0x05) && (0x64)) {
                        uint16_t dest = buf[5] << 0x08 | buf[4]; //To check
                        uint16_t src = buf[7] << 0x08 | buf[6]; //To check
                        NS_LOG_INFO("After I set the src and dst");
                        Lpdu::UserData data;
		        NS_LOG_INFO("After the data is set " << src << " " << dest);
                        data.dest = dest;
                        data.src = src;
		        NS_LOG_INFO("========================\n");
                        if (m_isMaster) {
			    ...
			    bool one_point = true;
                            if(one_point && ((testPack->GetSize() == 274) || (testPack->GetSize() >= 195))) {
			     std::vector<string> unique_id;
			     for(map<string, float>::iterator iter = analog_points.begin(); iter != analog_points.end(); iter++ ) {
                                 unique_id.push_back((*iter).first); //token);
		              }
			      for (int i = 0; i < unique_id.size(); i++){
                                  std::cout << "ID: " << unique_id[i] << std::endl;
			      }
			      std::cout << "Number of labels found " << unique_id.size() << std::endl;
			      int size_temp = buf.size();
			      //transforming our answer to bytes
			      Bytes buf_temp;
			      //Get index of point that we are trying to change the value of
			      ...
			      std::vector<std::string> val_max = get_val_vector (delimiter, m_attack_max);
                              //breaking down the the string to ints
				  ...
				  std::cout << "After the get_val" << std::endl;
				      ...
				      if (ind2 == 0 and i > 19){
                                          std::cout << "Found data crc check " << ind1 << " ID of point " << ID_point << std::endl;
				      }
				  
                                      if (( four > 0 ) && (four == ID_point) && (ind2 <= 18) && (i>19) && temp2[i] != 0x01){
                                          temp2[i] = cc[d]; 
				          d += 1;
				      }
				  }
				...
				std::cout << "The temp[12] = " << (int)temp2[12] << " packet size: " << testPack->GetSize() << " the last byte is " << (int)temp2[-1] << std::endl;
				for (int i = 0; i < buf.size(); i++){
                                    std::cout << (int)temp2[i] << ",";
				}
				std::cout << std::endl;
		   ...
		   if(buf.size() >= Lpdu::HEADER_SIZE) {
			   ...
			   if((start1 == 0x05) && (0x64)) {
				   ...
				   if (m_isMaster) {
					   ...
					   std::string delimiter = ",";
					   std::vector<std::string> val = get_val_vector (delimiter, m_attack_point_val);
					   std::vector<std::string> val_min;
					   std::vector<std::string> val_max;
                                           std::vector<std::string> nodes = get_val_vector (delimiter, node_id);
					   std::vector<std::string> points = get_val_vector (delimiter, point_id);
					   std::vector<std::string> nodesPoints;
					   std::cout << "node_id " << node_id << " " << nodes.size() << " " << points.size() << std::endl;
					   ...
					   for(map<string, float>::iterator iter = analog_points.begin(); iter != analog_points.end(); iter ++){
                                                unique_id_an.push_back((*iter).first);
					   }
					   ...
					   for (int qq = 0; qq < nodesPoints.size(); qq++){
					      ...
					      if (val[qq].find("TRIP") != std::string::npos){
					          ControlOutputRelayBlock ao(ControlOutputRelayBlock::Code::TRIP, ID_point);
					          m_p->direct_operate(false,ao);
					      }else if (val[qq].find("CLOSE") != std::string::npos){
                                                  ControlOutputRelayBlock ao(ControlOutputRelayBlock::Code::CLOSE, ID_point);
						  m_p->direct_operate(false, ao);
					      }else if (val[qq].find("LATCH_ON") != std::string::npos){
                                                  ControlOutputRelayBlock ao(ControlOutputRelayBlock::Code::LATCH_ON, ID_point);
						  m_p->direct_operate(false, ao);
					      }else if (val[qq].find("LATCH_OFF") != std::string::npos){
                                                  ControlOutputRelayBlock ao(ControlOutputRelayBlock::Code::LATCH_OFF, ID_point);
						  m_p->direct_operate(false, ao);
					      }
					   }
					   /*if (analog == true){ // need to find a way to set this to true during scenario 3 b
					       for (int i = 0; i < unique_id_an.size(); i++){ 
					          if (unique_id_an[i].find("load") != std::string::npos){
					              Bit32AnalogOutput ao(200*1000, i);
						      m_p->direct_operate(false,ao);
						    }
					        }
					   }*/
		  ...
		  if(buf.size() >= Lpdu::HEADER_SIZE) {
			  ...
			  if((start1 == 0x05) && (0x64)) {
				  ...
				  std::cout << "Dest: " << data.dest << " src: " << data.src << std::endl;
				  if (m_isMaster) {
					 NS_LOG_INFO("Am I the master----------------Insider");
				  } else {
					 //if ((int)temp[15] == 30 && (int)temp[16] == 5){
						 ...
						 std::cout << "f = " << f << std::endl; 
					         Bit32AnalogOutput ao(f*1000, ID_point);
					         m_p->direct_operate(false,ao);
					     }
					 //}
					 send_directly(packet);
...
void
Dnp3Application::Record (Ptr<Packet> packet, Address from)
{
 //NS_LOG_FUNCTION (this << socket);
 Address localAddress;
 if (packet)
...
/*
void
Dnp3Application::SetEndpointName (const std::string &name, bool is_global)
{
    ...
    func = std::bind (&Dnp3Application::EndpointCallback, this, _1, _2);
    helics_federate->setMessageNotificationCallback(m_endpoint_id, func);
}

void 
Dnp3Application::EndpointCallback (helics::Endpoint id, helics::Time time)
{
...
void 
Dnp3Application::DoEndpoint (helics::Endpoint id, helics::Time time)
{
...
void
Dnp3Application::DoEndpoint (helics::Endpoint id, helics::Time time, std::unique_ptr<helics::Message> message)
{
...
void
Dnp3Application::DoMessage (std::string target_endpoint, const std::string content, const std::string content_type)
{
	  NS_LOG_FUNCTION (this << target_endpoint << content);
	  std::string property_delimiter = "$";
	  std::vector<std::string> tokens = split(target_endpoint, property_delimiter);
	  ...
	  if (endsWith(new_target_endpoint, ".imag") || endsWith(new_target_endpoint,".real")) {
...
void
Dnp3Application::DoRead (std::unique_ptr<helics::Message> message)
{
